...
function appendCircle(parent, radius, className = "ring-line") {
  parent.appendChild(
    svgEl("circle", {
      cx: CX,
      cy: CY,
...
function appendRadials(parent, innerRadius, outerRadius, segments, offset = 0, className = "sector-line") {
...
function appendRingGrid(parent, innerRadius, outerRadius, segments, offset = 0, className = "ring-line") {
...
function appendTrigram(parent, angle, pattern) {
...
function buildStage3(parent) {
  ...
  const branchPairs = [...earthlyBranches, ...earthlyBranches];
  const stemPairs = Array.from({ length: 24 }, (_, index) => heavenlyStems[index % heavenlyStems.length]);
  appendTextRing(rotor, branchPairs, 240, 7.5, 16, "ring-text mid-text");
  appendTextRing(rotor, stemPairs, 268, 7.5, 13, "ring-text tiny-text");
...
function buildPrototype() {
  appendTaiji(document.getElementById("coreSpin"));
  buildStage2(document.getElementById("stage2"));
  buildStage3(document.getElementById("stage3"));
  buildStage4(document.getElementById("stage4"));
}
...
function attachInteraction() {
  ...
  const openWidgetPanel = () => {
    ornament.dispatchEvent(
      new CustomEvent("bagua-widget-open", {
        bubbles: true,
        detail: { source: "outer-icon" },
      }),
    );
  };
  ...
  ornament.addEventListener("click", openWidgetPanel);
}
